...
public class ParkingSpotController {
  ...
  public ResponseEntity<Object> saveParkingSpot(@RequestBody @Valid ParkingSpotDto parkingSpotDto) {
    ...
    if (parkingSpotService.existsByLicensePlateCar(parkingSpotDto.getLicensePlateCar())) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body("Conflict: License place car is already in use!");
    }
    ...
    if (parkingSpotService.existsByApartmentAndBlock(parkingSpotDto.getApartment(),
      ...
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body("Conflict: Parking spot already registered");
    }
  ...
  public ResponseEntity<Page<ParkingSpotModel>> getAllParkingSpots(
      @PageableDefault(page = 0, size = 3, sort = "block", direction = Direction.ASC)
      Pageable pageable) {
